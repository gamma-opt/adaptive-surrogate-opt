...
# sampling
data_GP = generate_data(goldstein_price, [Float32[-0.5, -1.5], Float32[0.5, -0.5]], 200, SobolSample(), 0.8)

# provide the configurations
config1_GP = NN_Config([2,1024,1], [relu, identity], false, 0.01, 0.4, Flux.Optimise.Optimiser(Adam(1, (0.89, 0.899)), ExpDecay(1, 0.08, 1000, 1e-4)), 1, 160, 5000)
config2_GP = NN_Config([2,512,1], [relu, identity], false, 0.001, 0.4, Adam(),  1, 160, 1000)
# config3_GP = NN_Config([2,512,1], [relu, identity], 0.001, 0.4, Adam(), 1000)
configs_GP = [config1_GP, config2_GP]
